...
namespace hrms.Application.Services.Vacation.SickLeaves.GetCurrentActiveSickLeaves
{
    public class GetCurrentActiveSickLeavesService : IGetCurrentActiveSickLeavesService
    {
        ...
        public GetCurrentActiveSickLeavesService(IRepository<SickLeaf> sickLeaveRepository, IRepository<HolidayType> holidayTypeRepository, IGetUserService getUserService, IQuartersCountsService quartersCountsService)
        {
        ...
        public async Task<ServiceResult<GetCurrentActiveLeavesServiceResponse>> Execute(int userId, CancellationToken cancellationToken)
        {
            ...
            var holidayTypeCode = VacationContants.HolidayCode_SickLeave;
            ...
            {
                throw new NotFoundException($"Holiday type or range type for sick leave not found");
            }
            ...
            //Get all sick leaves during previously calculated ranges
            var userSickLeaves = await _sickLeaveRepository
            ...
            if (holidayType.MaxAmountReservedDaysForAnotherUsageRange > 0 && holidayType.CanUseReservedDaysInAnotherRangeForDays != null && holidayType.CanUseReservedDaysInAnotherRangeForDays > 0)
            {
                if (isQuarterRange == true)
                {
                    ...
                    if (DateTime.Now <= availableforDays)
                    {
                        ...
                        {
                            //Get all used sick leave for previous quarter
                            var userSickLeavesInPreviousQuarter = await _sickLeaveRepository
                            ...
                            var totalUsedDaysForPreviusQuarter = userSickLeavesInPreviousQuarter.Sum(x => x.CountDays);
                            //Count left days
                            var leftSickLeavesDaysForPreviusQuarter = holidayType.CountUsageDaysPerRange - totalUsedDaysForPreviusQuarter < 0 ? 0 : holidayType.CountUsageDaysPerRange - totalUsedDaysForPreviusQuarter;
                            //Calculate remaining max available days
                            remainingAvailableDaysFromPastQuarterOrYear = leftSickLeavesDaysForPreviusQuarter > holidayType.MaxAmountReservedDaysForAnotherUsageRange
                                ? holidayType.MaxAmountReservedDaysForAnotherUsageRange
                                : leftSickLeavesDaysForPreviusQuarter;
                        }
                    }
                }
                else if (isQuarterRange == false)
                {
                    ...
                    if (DateTime.Now <= availableforDays)
                    {
                        if (user.Data?.UserProfileDTO.RegisterDate <= endDateForPreviusyear)
                        {
                            ...
                            var userPayedLeavesInPreviousYear = await _sickLeaveRepository
            ...
            {
                var totalUsedDays = userSickLeaves.Sum(x => x.CountDays);
