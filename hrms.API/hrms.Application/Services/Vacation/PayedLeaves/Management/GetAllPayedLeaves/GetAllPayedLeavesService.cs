...
{
    public class GetAllPayedLeavesService : IGetAllPayedLeavesService
    {
        private readonly IRepository<PayedLeaf> _payedLeavesRepository;

        public GetAllPayedLeavesService(IRepository<PayedLeaf> payedLeavesRepository)
        {
            _payedLeavesRepository = payedLeavesRepository;
        }

        public async Task<ServiceResult<List<PayedLeaveDTOWithUserDTO>>> Execute(GetAllPayedLeavesServiceFilter filter, CancellationToken cancellationToken)
        {
            var query = _payedLeavesRepository.GetAllAsQueryable();
            ...
            if (filter.PayBeforeHeadEnabled != null)
            {
                query = query.Where(x => x.PayBeforeHeadEnabled);
            }
...
#pragma warning disable CS8602 // Dereference of a possibly null reference.
            var result = await query
                ...
                .ConfigureAwait(false) ?? new List<PayedLeaveDTOWithUserDTO>();
#pragma warning restore CS8602 // Dereference of a possibly null reference.

            return ServiceResult<List<PayedLeaveDTOWithUserDTO>>.SuccessResult(result);
        }
    }
}
